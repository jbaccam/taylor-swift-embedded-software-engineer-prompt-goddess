...
// *
// * Uses IR sensor for edge detection and IR-based distance measurement
// * (instead of PING) for navigation to the smallest width object.
// *
// * @author Jeremiah Baccam
// * @date March 25, 2025
// */
//
//#include "Timer.h"
...
//#include "movement.h"
//#include "adc.h"  // Added for ADC functions
//
...
//float ping_values[NUM_POINTS];  // Now storing IR-based distance measurements
...
//
//// New helper functions for IR-based distance measurement
//float get_ir_distance(void);
...
//    cyBOT_init_Scan(0b0101);  // Enable servo, PING, and IR
...
//}
//
//// Perform a full scan and collect IR-based distance and IR data at each angle
//void scan_all_angles(void)
//{
//    right_calibration_value = RIGHT_CAL;
//    left_calibration_value = LEFT_CAL;
...
//    send_uart_string("Angle   IR Distance (cm)   IR Value\r\n");
//    send_uart_string("----------------------------------------\r\n");
//
...
//
//        // Instead of using scan.sound_dist, use IR-based distance measurement.
//        // The function get_ir_distance() performs averaging and conversion.
////        ping_values[i] = get_ir_distance();
//
//        // Keep the IR raw value for edge detection
//        ir_values[i] = scan.IR_raw_val;
//        ping_values[i] = calculate_distance(ir_values[i]); // ping_values[i] needs to be renamed to ir_distances or something
//
...
//}
//
//// Apply median filters to both IR distance and IR data to reduce noise
//void filter_sensor_data(void)
//{
...
//        // IR distance filter (float values)
...
//}
//
//// Median filter for float values (IR distances)
//float median_of_3_float(float a, float b, float c)
//{
//    if (a > b) {
...
//}
//
//// Detect objects using IR for edge detection and IR-based distance for distance measurement
//void detect_objects(void)
//{
...
//            }
//
//            // Find minimum IR-based distance within object boundaries
...
//            float centerAngle = (startAngle + endAngle) / 2.0f;
//
//            // Find minimum IR-based distance within object
//            float minDist = 999.0f;
//            for (j = startIndex; j <= endIndex; j++) {
//                if (ping_filtered[j] < minDist && ping_filtered[j] > 0) {
//                    minDist = ping_filtered[j];
...
//}
//
//// New function: Averaging ADC samples and calculating IR-based distance
//float get_ir_distance(void)
//{
//    int numSamples = 16;
//    int sum = 0;
...
//        sum += adc_read();
////        timer_waitMillis(5); // Small delay between samples
//    }
//    int avgAdc = sum / numSamples;
//    return calculate_distance(avgAdc);
//}
//
//// New function: Convert ADC value to distance in cm using a derived equation.
//float calculate_distance(int irValue)
//{
//    if (irValue <= 0) {  // Prevent division by zero or negative values
//        return 0;
//    }
//    // Directly compute the distance from the ADC reading using the derived equation:
